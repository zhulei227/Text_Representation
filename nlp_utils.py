...
class SegmentWordModel(object):
    def __init__(self, user_dicts_directory=None, stop_word_dicts_directory=None):
        '''
        ...
        if stop_word_dicts_directory is not None:
            ...
            for filename in filenames:
                for line in open(stop_word_dicts_directory + filename, encoding='utf-8-sig'):
                    self.stop_words.add(line.strip())

    def _clean_stop_words(self, seg_list):
        new_seg_list = []
    ...
    def cut_for_all(self, text):
        ...
        return ' '.join(self._clean_stop_words(seg_list))
    ...
    def cut_for_all_and_search(self, text):
        ...
        for item in jieba.cut(text, cut_all=True):
            seg_set.add(item)
        return ' '.join(self._clean_stop_words(seg_set))
    ...
    def cut_with_preprocess(self, text):
        ...
        return ' '.join(self._clean_stop_words(jieba.cut(text)))
...
class FeatureSelectModel(object):
    def __init__(self, keep_feature_num=1000, where_is_corpus=None, how_to_extract_text=None,
                 how_to_extract_label=None):
        """
    ...
    def set_bow_model(self, bow_model):
        ...
        if self.corpus_path is None:
            raise ValueError(
                'please set FeatureSelectModel\'s where_is_corpus param or set_bow_model() with where_is_corpus param')
    ...
    def update_bow_model_by_gini_select(self):
        estimator = RandomForestClassifier(criterion='gini', n_jobs=-1)
...
class CorpusModel(object):
    ...
    def __init__(self, where_is_corpus=None, how_to_extract_text=None, how_to_segment_word=None,
                 where_store_dictionary=None):
        """
...
class TopicModel(CorpusModel):
    ...
    def transfer(self, other_texts=None):
        ...
        if other_texts is not None:
            self.topic_model = self.topic_model_class(
                corpus=self.base_corpus_model.encode_corpus_texts(texts=other_texts), num_topics=self.topic_num)
        else:
            self.topic_model = self.topic_model_class(corpus=self.base_corpus_model.extract_corpus_data(),
...
class BOWModel(CorpusModel):
    ...
    def load_corpora_documents(self):
        if self.corpus_path is None:
            raise RuntimeError(
                "bow model\'s corpus path can not be None,you should set bow model\'s param,like:bow_model.where_is_corpus='./courpus.txt'")
        corpora_documents = []
        for line in open(self.corpus_path, encoding='utf8'):
    ...
    def update_corpus_model_with_feature_select(self, feature_select_model=None, how_to_select_feature='gini'):
        ...
        :return:
        '''
        feature_select_model.set_bow_model(self).update_bow_model_by_feature_select(
            how_to_select_feature=how_to_select_feature)
...
class TfidfModel(CorpusModel):
    ...
    def encode_ndarray_texts(self, texts):
        return gensim.matutils.corpus2dense(self.encode_corpus_texts(texts),
                                            num_terms=len(self.bow_model.dictionary)).transpose()
...
class LSIModel(TopicModel):
    def __init__(self, base_corpus_model, where_store_lsi_model, topic_num):
        ...
        :param topic_num:主题数
        """
        TopicModel.__init__(self, base_corpus_model=base_corpus_model, where_store_topic_model=where_store_lsi_model,
                            topic_num=topic_num, topic_model_class=LsiModel)


class Word2VecModel(CorpusModel):
    def __init__(self, where_is_corpus=None, how_to_extract_text=None, how_to_segment_word=None,
                 where_store_w2v_model=None, train_params=None):
        """
        ...
        """
        CorpusModel.__init__(self, where_is_corpus=where_is_corpus, how_to_extract_text=how_to_extract_text,
                             how_to_segment_word=how_to_segment_word, where_store_dictionary=None)
    ...
    def load_corpora_documents(self):
        if self.corpus_path is None:
            raise RuntimeError(
                "model\'s corpus path can not be None,you should set model\'s param,like:model.where_is_corpus='./courpus.txt'")
        corpora_documents = []
        for line in open(self.corpus_path, encoding='utf8'):
            text = self.extract_text_operator(line)
            corpora_documents.append(self.seg_operator(text).split(' '))
    ...
    def extract_corpus_data(self):
        raise RuntimeError('need not to implement!')
